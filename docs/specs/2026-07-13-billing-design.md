...
- **Price: $99/year** (~$8.25/mo equivalent). Chosen over a lower $50/year anchor to keep unit economics closer to the original spec's $19/mo (~$228/year) revenue target — $99 needs roughly 2.3x the paying-creator count to hit the same MRR target, versus ~4.6x at $50 — while still reading as a steep annual deal rather than a bargain-bin price for a product positioned as "insurance for your livelihood." The price lives entirely in LemonSqueezy's product/variant configuration (`LEMONSQUEEZY_VARIANT_ID`), not hardcoded in app logic — the only place it appears in code is display copy on `/pricing`. Raising the price later as the product grows is an operational change (update the LemonSqueezy variant + the displayed number), not a code change; whether to grandfather existing subscribers at $99 is a business decision to make at that time, out of scope here.
...
A new public route, `app/pricing/page.tsx`, shown before checkout so a creator knows exactly what a year of payment buys — the moment of highest hesitation for an annual-only plan. Linked from the dashboard's "Upgrade to unlock backup" card (which points here instead of straight to `/api/billing/checkout`) and reusable as a standalone marketing link.

Layout: the `Wordmark` primitive at the top, a short header ("Choose your plan" + the spec's existing tagline, "Resilience, never recovery. If your account vanishes, your business doesn't."), then two `Card`s side by side —

- **Free** — $0, checkmarks for breach alerts and break-glass status page, lock icons (muted text) for Instagram backup and the capture page.
- **Creator** — accented border (2px, the one deliberate exception to the app's 0.5px border convention, per the existing design system's rule for featured items) with a "Recommended" badge, "$99/year" with a "$8.25/mo, billed once annually" subline, checkmarks for all four features, and an "Upgrade to Creator" button linking to `/api/billing/checkout`.

A footer line under both cards: "Cancel anytime from your dashboard. No auto-upsells, no recovery-scam tactics — just a straight yearly plan." — reinforcing the spec's resilience-not-recovery positioning at the exact point of payment decision.

No new primitives needed beyond what `components/ui/` already has (`Shell`, `Card`, `Button`, `Badge`, `Wordmark`); this is a straightforward assembly of existing pieces.
